"""Multi-account OAuth authentication for Google Calendar."""
...
# Calendar API scopes - read-only for safety
SCOPES = [
...
class AuthManager:
    """Manages OAuth credentials for multiple Google accounts."""
    ...
    def get_credentials(self, account: str) -> Credentials:
        ...
        if not token_path.exists():
            raise AuthError(
                ...
                f"Run: gcalendar-mcp auth --account {account}"
            )
        ...
        if not creds.valid:
            raise AuthError(
                ...
                f"Re-authenticate: gcalendar-mcp auth --account {account}"
            )
